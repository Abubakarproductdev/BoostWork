...
export default function OverallPerformance() {
  ...
  const metrics = useMemo(() => {
    ...
    const statusData = [
      ...
      { name: 'Won', value: won, color: '#8b5cf6' }
    ];
  ...
  const statCards = [
    { label: 'Jobs Applied', value: metrics.applied, icon: Briefcase, color: 'text-blue-400', bg: 'bg-blue-400/10' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Analytics for the selected timeframe</p>
        </div>
        ...
        <div className="relative">
          <select 
            ...
            className="appearance-none bg-[#1e293b] text-white border border-[#334155] rounded-xl px-5 py-2.5 pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500/50 hover:bg-[#273549] transition-colors cursor-pointer font-medium shadow-sm transition-all"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {statCards.map((stat, i) => (
          <div key={i} className="glass-panel p-5 rounded-2xl relative overflow-hidden group hover:border-white/10 hover:bg-[#1e293b]/50 transition-all cursor-default">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={metrics.areaData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="viewed" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorViewed)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
